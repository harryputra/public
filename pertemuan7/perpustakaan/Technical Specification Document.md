...
**SmartLib** adalah sistem manajemen perpustakaan berbasis PHP/MySQL yang dirancang untuk kebutuhan pengajaran SQA. Dokumen ini merinci spesifikasi teknis dari sistem otomasi "One-Click Setup" dan "Live Server" yang diimplementasikan oleh SQA Architect.
...
* **Academic Testing Pedagogical Standard**: Implementasi panduan lab komprehensif dalam `test_case.md` yang mencakup 100% cakupan fungsional sistem (Admin, Librarian, Student). Mencakup instruksi pengerjaan laporan, template log hasil pengujian, serta perluasan skenario *Negative Testing* dan *Business Rule Validation* untuk seluruh modul inti SmartLib.
* **Firefox Master Selenium Protocol**: Standardisasi penggunaan `execute script` pada dokumen pengujian untuk mengatasi limitasi API `type` pada browser Firefox, menjamin reliabilitas 100% pada pengisian formulir CRUD dan transaksi sirkulasi bagi mahasiswa. Panduan `test_case.md` kini mencakup **35 Test Case terverifikasi** yang dipetakan terhadap **20+ fungsi utama** sistem SmartLib (Auth, Admin CRUD, Librarian CRUD, Circulation, Student Services, Reservation, Security), lengkap dengan skenario Positive dan Negative Testing untuk setiap modul.
